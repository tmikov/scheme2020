...
set(S2020_TOOLS_OUTPUT_DIR "${CMAKE_BINARY_DIR}/bin/${CMAKE_CFG_INTDIR}")
...
function(s2020_update_compile_flags name)
...
function(add_s2020_library name)
  cmake_parse_arguments(ARG "" "" "LINK_LIBS;LLVM_COMPONENTS" ${ARGN})
  add_library(${name} STATIC ${ARG_UNPARSED_ARGUMENTS})
  if (NOT ARG_LLVM_COMPONENTS STREQUAL "")
    llvm_config(${name} ${ARG_LLVM_COMPONENTS})
  endif()
  target_link_libraries(${name} PUBLIC ${ARG_LINK_LIBS} ${S2020_LINK_COMPONENTS})
  set_property(TARGET ${name} PROPERTY POSITION_INDEPENDENT_CODE ON)
  s2020_update_compile_flags(${name})
endfunction(add_s2020_library)

function(add_s2020_executable name)
  cmake_parse_arguments(ARG "" "" "LINK_LIBS;LLVM_COMPONENTS" ${ARGN})
  ...
  target_link_libraries(${name} PUBLIC ${ARG_LINK_LIBS} ${S2020_LINK_COMPONENTS})
  s2020_update_compile_flags(${name})
endfunction(add_s2020_executable)

function(add_s2020_tool name)
  add_s2020_executable(${name} ${ARGN})

  # In multi-config build systems, remove the per-config directory and let
  # CMake automatically add it in the component.
  if (NOT "${CMAKE_CFG_INTDIR}" STREQUAL ".")
    string(REPLACE ${CMAKE_CFG_INTDIR} "" S2020_TOOLS_OUTPUT_DIR "${S2020_TOOLS_OUTPUT_DIR}")
  endif ()

  set_property(TARGET ${name} PROPERTY RUNTIME_OUTPUT_DIRECTORY
    "${S2020_TOOLS_OUTPUT_DIR}")
endfunction(add_s2020_tool)
